...
(function () {
    ...
    function nodeManager(translateNodeService, translateJobService) {
        ...
        function createAllNodes(contentNodes, includeChildren, options, batchSize,
            msgCallback,
            completeCallback,
            errorCallback) {
            msgCallback({
            ...
            var nodeList = contentNodes.map(function (x) {
                return {
                    id: x.id,
                    path: x.path,
                    tm_includeChildren: x.tm_includeChildren || includeChildren
                }
            });

            translateNodeService.getNodesFromList(nodeList)
            ...
            function process(items) {
                ...
                translateNodeService.createNodes(items, options)
                    .then(function (response) {
                        ...
                        if (attempt.success) {
                            nodes = nodes.concat(attempt.result);
                            if (b < batches.length) {
                                process(batches[b]);
                            }
                            else {
