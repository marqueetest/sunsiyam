(function () {
    ...
    function createDialogController($scope,
        ...
        if ($scope.model.items !== undefined && $scope.model.items.length > 0) {
            vm.multipleItems = true;
            vm.items = $scope.model.items;
            vm.currentNodeId = vm.items[0].id;
            vm.currentNodeName = "Bulk Translation";
            vm.children = false; 
        }
        else {
            vm.items = [$scope.model.entity];
        ...
        vm.performTargetCheck = Umbraco.Sys.ServerVariables.translationManager.options.checkTargets;
        ...
        function createNodes(createJobWhenComplete) {
            ...
            translateNodeManager.createAllNodes(vm.items, vm.children, options, vm.batchSize, showMessage,
                function (nodes) {
        ...
        vm.missingTargets = false;
        vm.validTargets = false; 

        function checkTargets(nodeId) {

            if (vm.performTargetCheck) {

                translateSetService.checkTargets(nodeId)
                    .then(function (result) {
                        vm.targetStatus = result.data;

                        console.log(vm.targetStatus);

                        _.forEach(vm.sets, function (set) {
                            _.forEach(set.Sites, function (site) {
                                var target = _.find(vm.targetStatus, function (t) {
                                    return (t.SiteId == site.Id && t.SetId == set.Id);
                                });

                                if (target != undefined) {
                                    site._invalid = !target.Valid;

                                    if (site._invalid) {
                                        vm.missingTargets = true;
                                    }
                                    else {
                                        vm.validTargets = true;
                                    }
                                }
                            })
                        });
